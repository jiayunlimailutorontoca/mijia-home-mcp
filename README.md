...
### watch 通知:小爱播报 / webhook

变化不只打印在终端,还可以推出去:

```bash
# 有变化时让小爱音箱播报(play-text 纯 TTS,不会触发指令执行)
mijia-home-mcp watch --speak --speaker-name "Xiaomi Smart Speaker"

# 有变化时把 diff JSON POST 到你的服务(Bark/ntfy/自建均可)
mijia-home-mcp watch --webhook https://example.com/hook

# 噪音控制:只关注某些设备 / 忽略某些设备或属性
mijia-home-mcp watch --speak --only "门锁*" --only "*传感器*" --ignore left-time
```
...
```json
{
  ...
  "since": "2026-07-12T14:26:04+08:00",
  "until": "2026-07-12T14:27:04+08:00",
  "change_count": 1,
  "changes": [
...
`text` 字段是现成的一句话摘要,接 Bark/ntfy 这类推送服务可以直接用。通知失败(音箱离线/网络抖动)只记录不中断监控。
